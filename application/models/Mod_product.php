...
class Mod_product extends CI_Model
{
    ...
    // 確認商品庫存是否足夠
    public function chk_reserve($id, $qty)
    {
        $this->db->where('id', $id);
        $res = $this->db->get('product_main')->row_array();
    ...
    // 更新庫存
    public function update_reserve($id, $qty)
    {
        $this->db->where('id', $id);
        $res = $this->db->get('product_main')->row_array();
        ...
        if ($res['reserve'] >= $qty)
        {
            ...
            $newQty = $oldQty - $qty;
            $this->db->where('id', $id);
            return $this->db->update('product_main', array('reserve' => $newQty));
        } else {
            return false;
        }
    }
